...
module.exports.getSocket = (playerID) => {
  if(player[playerID].socket != null) return player[playerID].socket;
  return false;
}
...
module.exports.ready = (playerID) => {
  ...
  return {both: (match[matchID].host.ready && match[matchID].opponent.ready) ? true : false, ready: isReady};
}
...
module.exports.start = (playerID) => {
  ...
  return (match_.host.ready && match_.opponent.ready) ? true : false;
}
