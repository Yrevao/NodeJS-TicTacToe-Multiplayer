...
const newPlayer = (matchID, isHost) => {
  return {
    ...
    host: isHost,
    socket: {
      emit: () => { return 0 }  // placeholder emit function for unit tests
    },
  }
}
...
module.exports.setSocket = (playerID, socket) => {
  if(player[playerID]) player[playerID].socket = socket;
}
module.exports.getSocket = (playerID, socket) => {
  if(player[playerID]) return(player[playerID].socket);
}

// a match has two players in it, return the playerID of the additional player in the same match as the argument player
module.exports.getOtherPlayer = (playerID) => {
  const player_ = player[playerID];
  if(player_) {
    const match_ = match[player_.match];
    return player_.host ? match_.opponent.ID : match_.host.ID;
  }
  return false;
}

// get matchID with playerID
...
module.exports.prune = (playerID) => {
  if(player[playerID]) {
    ...
    if(match[player_.match].opponent.ID != null) {
      ...
      player[match[player_.match].opponent.ID].host = true;
